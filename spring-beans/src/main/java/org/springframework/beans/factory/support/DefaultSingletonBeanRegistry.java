...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * 这里是判断当前需要创建的bean是否在Exclusions集合，被排除的bean，程序员可以提供一些bean不被spring初始化（哪怕被扫描到了，也不初始化），那么这些提供的bean便会存在这个集合当中；一般情况下我们不会提供，而且与循环依赖无关；故而所以这里不做深入分析，后面文章如果写到做分析；
	 */
	private final Set<String> inCreationCheckExclusions =
			Collections.newSetFromMap(new ConcurrentHashMap<>(16));

	/**
	 * Collection of suppressed Exceptions, available for associating related causes. 禁止显示的异常集合，可用于关联相关原因。
	 */
	@Nullable
	...
	protected void beforeSingletonCreation(String beanName) {
		// 这个 bean 没有被排除并且这个 bean 当前正在被创建, 抛出异常
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		/*
		dependentBeanMap - dependentOn 内的 Bean 将作为 key, 而本身 dependentOn 这个 Bean 的 Bean 将作为 value
		 */
